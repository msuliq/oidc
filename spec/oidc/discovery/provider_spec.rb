...
describe Oidc::Discovery::Provider do
  let(:provider) { 'https://server.example.com' }
  let(:discover) { Oidc::Discovery::Provider.discover! identifier }
  let(:endpoint) { "https://#{host}/.well-known/webfinger" }
  let(:query) do
    {
      rel: Oidc::Discovery::Provider::Issuer::REL_VALUE,
      resource: resource
    }
  end
  ...
  describe '#discover!' do
    ...
    context 'when error occured' do
      let(:identifier) { host }
      let(:resource)   { "https://#{host}" }
      it 'should raise Oidc::Discovery::DiscoveryFailed' do
