...
describe Oidc::Discovery::Provider::Config do
  ...
  describe 'discover!' do
    it 'should setup given attributes' do
      mock_json :get, endpoint, 'discovery/config' do
        config = Oidc::Discovery::Provider::Config.discover! provider
        config.should be_instance_of Oidc::Discovery::Provider::Config::Response
    ...
    context 'when OP identifier includes custom port' do
      ...
      it 'should construct well-known URI with given port' do
        mock_json :get, endpoint, 'discovery/config_with_custom_port' do
          Oidc::Discovery::Provider::Config.discover! provider
        end
      end
    end
    ...
    context 'when SWD::Exception raised' do
      it do
        ...
        end.to raise_error Oidc::Discovery::DiscoveryFailed
      end
    end
    ...
    describe 'when response include invalid issuer' do
      ...
      context 'when issuer validation is disabled.' do
        before :each do
          Oidc.validate_discovery_issuer = false
        end

        after :each do
          Oidc.validate_discovery_issuer = true
        end
