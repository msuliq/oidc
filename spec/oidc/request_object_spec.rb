...
describe Oidc::RequestObject do
  subject { request_object }
  let(:request_object) { Oidc::RequestObject.new attributes }
  ...
  context 'with all attributes' do
    ...
    describe '.decode' do
      it do
        Oidc::RequestObject.decode(jwtnized, 'secret').as_json.should == jsonized.with_indifferent_access
      end
    end
    ...
    describe '.fetch' do
      ...
      it do
        mock_json :get, endpoint, 'request_object/signed', format: :jwt do
          request_object = Oidc::RequestObject.fetch endpoint, 'secret'
